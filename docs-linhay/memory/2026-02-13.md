...
## ACP 扩展边界守卫（追加）
- 决策：runtime/policy/telemetry 扩展统一标注为 Non-ACP extension，并新增协议载荷污染测试。
- 实现：
  - `ACPRuntime`、`ACPClientService+Runtime`、`ACPPermissionPolicy` 增加边界注释；
  - 新增 `ACP-Protocol-Extension-Boundaries` feature 文档；
  - 新增 `testTelemetryExtensionDoesNotLeakIntoProtocolPayload` 守卫测试。

## WebSocket 回归稳定化（追加）
- 决策：将 WebSocket 集成测试端口分配统一收敛到可重试 harness，避免并发下端口碰撞导致 flaky。
- 实现：
  - 新增 `ACPWebSocketTestHarness`（随机端口重试 + 固定端口重试绑定）；
  - 新增 `ACPWebSocketTestHarnessTests`；
  - 五个 WebSocket 集成测试切换到 harness 分配端口。
- 运维：
  - 新增 `docs-linhay/plans/ops/ACP-WebSocket-Test-Stability-Runbook.md`。
- 回归：
  - 目标 WebSocket/Transport 用例全部通过。

## Session Export 扩展与 Fork 谱系元数据（追加）
- 决策：新增 `session/export` 作为 project extension（非 ACP stable/unstable 方法），用于对标 pi 的会话导出维护能力。
- 边界：扩展方法进入 `ACPMethodCatalog.projectExtensions`，并在 feature/dev 文档明确“扩展域”归属。
- 实现：
  - `ACPMethods.sessionExport` 与 `ACPSessionExport*` 模型；
  - `ACPSessionCapabilities.export` 能力位门禁；
  - `ACPAgentService` 支持导出 JSONL（header + message lines）；
  - `session/fork` 记录 `parentSessionId`，`session/list` 返回 `parentSessionId/messageCount`；
  - `ACPClientService` 新增 `exportSession(_:)`；
  - `acp serve` 默认声明 `sessionCapabilities.export`。
- 回归：
  - `testSessionExportRequiresCapability`
  - `testSessionExportReturnsJSONLContent`
  - `testSessionListIncludesParentSessionIdForForkedSession`
  - `testSessionExportRoundTrip`
  - `testProjectExtensionMethodsAreExplicitlyScoped`
  - `swift test --filter ACP --parallel` 通过。
