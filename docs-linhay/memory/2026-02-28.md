...
- 实现：
  ...
  - 新增 docs 入口：`docs-linhay/dev/Skills-Index.md`，并在 `docs-linhay/dev/Getting-Started.md` 增加索引链接。
- 验证：
...
- 新增：
  ...
  - 更新：`skills/skintelligence/SKILL.md` 与 `docs-linhay/dev/Skills-Index.md`。
- 验证：
...
- 变更：
  - `docs-linhay/dev/Codex-ACP-Runbook.md` 增加技能入口（SKILL + ACP 回归/smoke 脚本）。
  - `docs-linhay/dev/testing-strategy.md` 增加技能测试入口（targeted/library smoke 脚本）。
- 验证：
  - 技能校验与打包通过；
...
## 发布收尾：STJSON 迁移运维发布说明
- 新增运维发布文档：`docs-linhay/plans/ops/Release-2026-02-28-STJSON-Migration.md`。
- 内容包含：迁移范围、关键删除项、全量回归结果（420/0/1）、关联提交、风险提示。
- 目的：为后续发布审计与回滚评估提供单一入口。

## 发布：1.3.18
- 已创建并推送 tag：`1.3.18`。
- 已创建 GitHub Release：
  - https://github.com/linhay/SKIntelligence/releases/tag/1.3.18
- 发布摘要：STJSON JSON-RPC 迁移收尾（移除 SKIJSONRPC、去除 AnyCodable JSONValue 风格兼容糖衣、全量回归通过）。

## 发布文档补齐：1.3.18
- 新增 `docs-linhay/plans/ops/Release-1.3.18.md`，将 tag/release/验证结果与提交列表按版本固化。
