...
- 规格同步：`docs-linhay/features/SKIAgentSession-Spec.md` 新增场景 19（空闲态 fire-and-forget resolved/failed 历史一致性）。
...
- 文档同步：`docs-linhay/dev/testing-strategy.md` 已更新矩阵覆盖项，移除 fork prompt 缺口描述（已实测纳入）。
...
- 文档同步：`docs-linhay/dev/testing-strategy.md` 已增加上述两条矩阵契约描述。
...
- 新增覆盖矩阵文档：`docs-linhay/dev/ACP-Spec-Coverage-Matrix.md`（spec 场景 -> 测试映射）。
