...
- 已将上游仓库拉取到 `docs-linhay/references/Wax`，用于后续源码级对照和离线审阅。
- 新增文档：`docs-linhay/dev/Wax-Single-File-Memory-Architecture.md`。
...
- 已在 `docs-linhay/dev/Wax-Single-File-Memory-Architecture.md` 增补 `MiniLM-L6-v2.mlmodelc` 调用链路：
...
- 迁移文件：
  ...
  - `docs-linhay/dev/Streaming.md`
  - `docs-linhay/plans/ops/Release-1.3.17.md`
- 结果：相关测试编译与执行通过，避免继续触发已弃用 API 的调用告警。

## Codex Skills 机制沉淀
- 决策：将 openai/codex 长期沉淀到 docs-linhay/references/openai-codex（commit: 6a673e7），不再仅放 /tmp。
- 产出：新增分析文档 docs-linhay/references/docs/dev/openai-codex-skills-implementation-analysis.md，覆盖 skills 的发现、加载、触发、注入、依赖与协议实现。
