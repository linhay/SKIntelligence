...
- summary 新增 `nonPassOptionalStages`，便于直接提取“可选阶段非通过”集合做告警聚合。

- summary 新增 `optionalPassStages`，用于快速识别本轮通过的可选探针。

- summary 新增 `hasOptionalNonPass`（boolean），调用方可直接判断可选阶段是否存在非通过。

- summary 新增 `hasRequiredFailures`（boolean），便于直接判断是否存在必跑失败。

- summary 新增 `optionalOutcome`（`clean | degraded`），简化下游对可选阶段整体状态的判断。

- summary 新增 `requiredOutcome`（`clean | blocked`），与 `optionalOutcome` 形成对称聚合语义。

- summary 新增 `overallOutcome`（`clean | degraded | blocked`），便于下游一键判定总体状态。

- summary 新增 `overallOutcomeRank`（0/1/2），用于告警与报表系统直接分级处理。
