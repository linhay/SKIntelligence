...
本页提供项目技能入口。`docs-linhay` 可以引用 `skills`，但 `skills` 不反向引用 `docs-linhay`。
...
1. 只允许 `docs-linhay/* -> skills/*` 单向引用。
2. 技能内方法必须自包含，不依赖 `docs-linhay` 路径跳转。
3. 项目需求、架构与治理文档继续以 `docs-linhay` 为主。
