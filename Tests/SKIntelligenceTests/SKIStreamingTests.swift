...
final class SKIStreamingTests: XCTestCase {
    ...
    // MARK: - ServerEventParser Tests

    func testParseSimpleDataEvent() {
    ...
    func testParseEventWithEventType() {
        var parser = ServerEventParser()
        ...
        XCTAssertEqual(events.first?.data, "{\"text\": \"Hello\"}")
    }
    ...
    func testParseDoneSignal() {
        ...
        XCTAssertEqual(events.count, 1)
        XCTAssertTrue(events.first?.isDone ?? false)
    }

    func testBufferIncompleteMessage() {
        var parser = ServerEventParser()

        // First chunk - incomplete
        let chunk1 = "data: {\"part\": ".data(using: .utf8)!
        let events1 = parser.parse(chunk1)
        XCTAssertEqual(events1.count, 0)  // Should buffer, not return

        // Second chunk - completes the message
        let chunk2 = "1}\n\n".data(using: .utf8)!
        let events2 = parser.parse(chunk2)
        XCTAssertEqual(events2.count, 1)
        XCTAssertEqual(events2.first?.data, "{\"part\": 1}")
    }
    ...
    func testIgnoreCommentLines() {
        ...
        XCTAssertEqual(events.count, 1)
        XCTAssertEqual(events.first?.data, "actual data")
    }
    ...
    func testParserReset() {
        ...
        // Add incomplete data
        let _ = parser.parse("data: incomplete".data(using: .utf8)!)

        // Reset parser
        parser.reset()

        // Parse new complete data
        let events = parser.parse("data: fresh\n\n".data(using: .utf8)!)

        XCTAssertEqual(events.count, 1)
        XCTAssertEqual(events.first?.data, "fresh")
    }
