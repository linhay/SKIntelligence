...
SKIP_ORG_SCAN="${ACP_UPSTREAM_SKIP_ORG_SCAN:-0}"
...
> "$repo_sections"
priority_items="$tmp_dir/priority-items.tsv"
> "$priority_items"
...
while IFS= read -r repo; do
  ...
  jq --arg repo "$repo" --argjson days "$SINCE_DAYS" -r '
    .[]
    | select((now - (.updated_at|fromdateiso8601)) <= ($days*86400))
    | ((.title // "") + " " + (.body // "")) as $text
    | if ($text | test("breaking|deprecat|protocol|schema|json-rpc|transport"; "i")) then
        "P0\t\($repo)\t\(.updated_at // "unknown")\t\((.title // "untitled") | gsub("[\r\n\t]+";" "))\t\(.html_url // "")"
      elif ($text | test("sdk|client|server|example|tool|session|permission"; "i")) then
        "P1\t\($repo)\t\(.updated_at // "unknown")\t\((.title // "untitled") | gsub("[\r\n\t]+";" "))\t\(.html_url // "")"
      else
        empty
      end
  ' "$prs_file" | head -n 10 >> "$priority_items" || true
...

{
  ...
  if [ "$p0_hits" -gt 0 ]; then
    echo "- Action: same-day P0 assessment required; see \`Priority Items\` and run ACP regression suite."
  elif [ "$p1_hits" -gt 0 ]; then
    echo "- Action: include P1 items in weekly alignment and track owners."
  else
  ...
  echo "## Priority Items"
  echo
  if [ -s "$priority_items" ]; then
    while IFS=$'\t' read -r level repo_id updated title url; do
      [ -z "$level" ] && continue
      echo "- Level: ${level}"
      echo "- Repo: ${repo_id}"
