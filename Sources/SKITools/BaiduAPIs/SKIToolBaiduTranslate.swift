...
public struct SKIToolBaiduTranslate: SKITool {
    ...
    public struct Arguments: Codable, Sendable {
        ...
        @SchemaOptions(.description("Whether to enable text-to-speech. tts=0 shows, tts=1 does not show."))
        public var tts: Bool?
        /// [仅开通了词典者需填写] 是否显示词典资源 dict=0显示，dict=1不显示
        @SchemaOptions(.description("Whether to enable dictionary resources. dict=0 shows, dict=1 does not show."))
        public var dict: Bool?
        /// [仅开通“我的术语库”用户需填写] 判断是否需要使用自定义术语干预API, 1-是，0-否
        @SchemaOptions(.description("Whether to use custom terminology intervention API. 1-Yes, 0-No."))
    ...
    public func call(_ arguments: Arguments) async throws -> ToolOutput {
        var components = URLComponents(url: URL(string: authentication.host)!, resolvingAgainstBaseURL: true)
        ...
        let (data, response) = try await URLSession.tools.data(for: request)
